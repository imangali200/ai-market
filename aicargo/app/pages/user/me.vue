...
async function sendComment() {
    ...
    try {
        ...
        const targetPosts = allPosts.filter(p => p.id === activePostId.value)
...
const activePost = computed(() => {
    ...
    const allPosts = [...(profile.value.posts || []), ...(profile.value.postLikes || []), ...(profile.value.saved || [])]
    return allPosts.find(p => p.id === activePostId.value) || null
})
